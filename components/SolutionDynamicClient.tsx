...
export default function SolutionDynamicClient({ name, tagline, coverImageUrl, bodyContent, processSteps, slug }: Props) {
    ...
    try {
      await fetch(SUPABASE_URL, {
        ...
        body: JSON.stringify({
          ...
          source: `solution_${slug.replace(/-/g, '_')}`,
          status: 'new',
  ...
  const heroBg = coverImageUrl || '/media/zeus-xr-21.jpg'

  return (
    <>
      {/* HERO */}
      <section className="sol-hero">
        <div className="sol-bg" style={{ backgroundImage: `url(${heroBg})` }} />
        <div className="sol-grad" />
        <div className="sol-content">
          <p className="sol-eyebrow">Solutions — Todd Engineering</p>
          <h1 className="sol-h1">{name}</h1>
          {tagline && <p className="sol-tagline">{tagline}</p>}
        ...
        <section className="intro-sec">
          <div className="intro-grid">
            <div className="rv">
              ...
              <div className="s-p">{bodyContent}</div>
          ...
          <div className="w-1300">
            ...
            <div className="process-grid">
              {processSteps.map((s, i) => (
                <div key={i} className={`proc-card rv${i > 0 ? ` d${i}` : ''}`}>
      ...
      <section className="sec sec-alt">
        <div className="w-1300">
          ...
          <div className="why-grid">
            {[
              { title: 'British Engineers', desc: 'All work carried out by our own in-house engineering team. No subcontractors, no handoffs.' },
              { title: 'Nationwide Coverage', desc: 'We operate across the UK with rapid response times and a network that covers every region.' },
              { title: '30+ Years Experience', desc: 'From small bodyshops to aerospace facilities — we have delivered it all since 1993.' },
            ].map((c, i) => (
      ...
      <section className="cform-sec" id="contact">
        <div className="cform-inner">
          <div className="rv">
            ...
            <h2 className="cform-h2">Interested in our {name} service?</h2>
